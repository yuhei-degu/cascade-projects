...
import { createMarketplaceEstimate } from "@/lib/estimate";
import type { MarketplaceEstimate } from "@/lib/estimate";
...
export default function NewRequestPage() {
  ...
  const estimate = useMemo(() => createMarketplaceEstimate(f), [f]);
    ...
    <div className="min-h-screen py-10 px-4">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mt-4 mb-8">
          <h1 className="text-2xl font-black mb-1">依頼を投稿する</h1>
          <p className="text-slate-400 text-sm">AIが審査→試作生成まで無料。OKなら決済へ進みます。</p>
        </div>
...
function EstimateCard({ estimate }: { estimate: MarketplaceEstimate }) {
  const fitClass = {
    strong_match: "border-emerald-500/30 bg-emerald-500/10 text-emerald-300",
    needs_clarification: "border-amber-500/30 bg-amber-500/10 text-amber-300",
    scope_down: "border-red-500/30 bg-red-500/10 text-red-300",
  }[estimate.fit];

  return (
    <aside className="lg:sticky lg:top-6 rounded-2xl border border-slate-800 bg-slate-900 p-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-xs font-semibold text-slate-500">概算プレビュー</p>
          <h2 className="mt-1 text-lg font-black">送信前の目安</h2>
        </div>
        <span className={`rounded-full border px-2.5 py-1 text-xs font-bold ${fitClass}`}>
          {estimate.fitLabel}
        </span>
      </div>

      <p className="mt-3 text-sm leading-relaxed text-slate-400">{estimate.fitDescription}</p>

      <div className="mt-5 grid grid-cols-3 gap-2 text-center">
        <Metric label="概算" value={`¥${estimate.estimatedPrice.toLocaleString()}`} />
        <Metric label="工数" value={`${estimate.estimatedHours}h`} />
        <Metric label="納期" value={`${estimate.deliveryDays}日〜`} />
      </div>
      ...
      <div className="mt-5">
        ...
        <div className="h-2 overflow-hidden rounded-full bg-slate-800">
          <div className="h-full rounded-full bg-violet-500" style={{ width: `${estimate.confidence}%` }} />
        </div>
      </div>

      <div className="mt-5 space-y-4 text-sm">
        <InfoList title="マッチ理由" items={estimate.matchReasons} />
        <InfoList title="初回版に含めやすいもの" items={estimate.includedItems} />
        <InfoList title="確認できると早いこと" items={estimate.questions} />
      </div>

      <div className="mt-5 rounded-xl border border-slate-800 bg-slate-950 p-3 text-xs leading-relaxed text-slate-500">
        正式な金額はAI審査と試作後に確定します。希望予算は{estimate.budgetLabel}、カテゴリは{estimate.categoryLabel}として見ています。
      </div>
    </aside>
  );
}
