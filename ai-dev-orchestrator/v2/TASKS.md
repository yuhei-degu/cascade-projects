...
- [x] 【磨き込み】残り画面にデザイントークンを適用する: インベントリ・キャラクター・記憶・アイテム・ゲームの各ページを、チャット/ガチャ/日記/ログインと同じダーク+ゴールドの世界観(背景#080706系、アクセント#f1c77f系)に統一し、各ページに日本語の空状態表示(データが無いときの案内文とアイコン)とローディング表示(スケルトンまたはスピナー)を整備する。機能変更はしない
- [x] 【磨き込み】エラー時のUXを整備する: 全ページのfetch失敗・API失敗時に、握りつぶしや英語のままのメッセージを排除し、日本語のわかりやすいエラー表示(例: 通信に失敗しました。もう一度お試しください)と再試行ボタンを共通コンポーネントとして実装して適用する。チャット送信失敗時は入力内容を消さずに残す
- [x] 【磨き込み】コードの贅肉を落とす: lib/ai.ts の未使用の callClaude と Anthropic クライアント、lib/prompt.backup.ts などのデッドコードを削除し、@anthropic-ai/sdk が他で未使用なら依存からも外す。/api/health は認証なしで200を返す純粋な死活監視エンドポイントに修正する。開発用のconsole.log残骸を主要ルートから整理する。ビルドが通ることを必ず確認
