...
要点：
- `/memory` で全 active 記憶をカード表示
- 日付フィルター + `/diary?date=...` への往来
- confirm（覚えてて）+ archive（棚から外す）+ restore で記憶ガバナンス完結
- 訂正は inline edit + confidence 自動降格
- 「外したもの」セクションで論理削除を可視化、復元可能
- `/diary` の `memory_changes` 見出しを「ルナが拾ったかけら」に変えて差別化
- ヘッダーコピーで「外してもいい」を最初に提示し、ユーザーの編集権を明確にする

実装ステップは D4a → D4b → D4c → D4d の 4 段。`NEXT_IMPLEMENTATION_QUEUE_2026-05-04.md` #4 / #5 が D4a / D4b に対応する。D4c（訂正）と D4d（diary 連動）は #5 完了後に追加キューとして検討。

「ルナの引き出し」の最初の引き出しを開ける、そのところまで持っていく設計です。
